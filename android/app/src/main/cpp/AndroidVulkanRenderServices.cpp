...
std::unordered_set<std::string> MissingUiTextures;

void ClearUiTextureCache()
{
    UiTextureCache.clear();
    MissingUiTextures.clear();
}
    ...
    AndroidVulkanUiTexture& texture, std::uint32_t& width, std::uint32_t& height)
{
    ...
    const auto cached = UiTextureCache.find(requestedName);
    if (cached != UiTextureCache.end())
    {
    ...
    if (MissingUiTextures.find(requestedName) != MissingUiTextures.end())
        return false;
    ...
    if (!found)
    {
        MissingUiTextures.emplace(requestedName);
    ...
    UiTextureCache.emplace(requestedName, CachedUiTexture{texture, width, height});
    return true;
}
...
bool AndroidLoadVulkanTexture(AndroidVulkanRenderDevice& renderDevice, const char* textureName,
    AndroidVulkanUiTexture& texture, std::uint32_t& width, std::uint32_t& height)
{
    return LoadUiTexture(renderDevice, textureName, texture, width, height);
}
