...
IC Fvector const* stalker_movement_params::desired_position() const { return (m_desired_position); }
IC void stalker_movement_params::desired_direction(Fvector const* direction)
{
    ...
    VERIFY(_valid(m_desired_direction_impl));
    VERIFY(!fis_zero(m_desired_direction_impl.square_magnitude()));
    // Original ShoC scripts deliberately use the vector magnitude as part of
    // the movement command. CScriptGameObject therefore preserves it in
    // ShadowOfChernobylMode, while the common debug invariant historically
    // still required a unit vector and aborted otherwise. Keep the strict
    // invariant for the other games, but honor ShoC's authored command data.
    VERIFY(ShadowOfChernobylMode || fsimilar(m_desired_direction_impl.magnitude(), 1.f));
    m_desired_direction = &m_desired_direction_impl;
}

IC Fvector const* stalker_movement_params::desired_direction() const { return (m_desired_direction); }
IC shared_str const& stalker_movement_params::cover_id() const { return (m_cover_id); }
IC smart_cover::cover const* stalker_movement_params::cover() const { return (m_cover); }
...
IC CGameObject const* stalker_movement_params::cover_fire_object() const { return (m_cover_fire_object); }
