...
void CUIZoneMap::Init(bool motionIconAttached)
{
    ...
    if (IsGameTypeSingle())
    {
        // Retail Shadow of Chernobyl zone_map.xml files do not all contain
        // the contact counter added by the newer OpenXRay UI layouts.
        if (uiXml.NavigateToNode("minimap:static_counter", 0))
        {
            ...
            Msg("~ ShoC minimap: optional contact counter is absent in %s", uiXml.m_xml_file_name);
    }
}
