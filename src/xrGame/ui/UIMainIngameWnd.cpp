...
void CUIMainIngameWnd::Init()
{
    ...
    if (uiXml.NavigateToNode("main", 0))
    {
        CUIXmlInit::InitWindow(uiXml, "main", 0, this);
    }
    else if (ShadowOfChernobylMode && uiXml.GetRoot())
    {
        // Retail ShoC maingame[_16].xml uses <window> itself as the document
        // root (and names its dimensions w/h), while later game data wraps a
        // child named <main>.  Keep all subsequent lookups rooted inside that
        // retail window and initialize its bounds without requiring a data mod.
        XML_NODE root = uiXml.GetRoot();
        const Fvector2 pos = {
            uiXml.ReadAttribFlt(root, "x", 0.0f),
            uiXml.ReadAttribFlt(root, "y", 0.0f),
        };
        const Fvector2 size = {
            uiXml.ReadAttribFlt(root, "w", UI_BASE_WIDTH),
            uiXml.ReadAttribFlt(root, "h", UI_BASE_HEIGHT),
        };
        SetWndPos(pos);
        SetWndSize(size);
        uiXml.SetLocalRoot(root);
        Msg("~ ShoC HUD: using retail root-window schema from %s", uiXml.m_xml_file_name);
    }
    else
    {
        CUIXmlInit::InitWindow(uiXml, "main", 0, this);
...
*/ //---------------------------------------------------------
    ...
    */
    ...
    // The retail ShoC HUD has no disk I/O indicator.  It is cosmetic and
    // later game layouts add it, so keep it optional for the original schema.
    ...
    if (uiXml.NavigateToNode("hud_states", 0))
        m_ui_hud_states->InitFromXml(uiXml, "hud_states");
    else if (ShadowOfChernobylMode)
        m_ui_hud_states->InitFromXml(uiXml, nullptr);
    else
        m_ui_hud_states->InitFromXml(uiXml, "hud_states");
...
void CUIMainIngameWnd::Draw()
{
    ...
    if (UIStaticDiskIO)
    {
        ...
        {
            u32 alpha =
                clampr(iFloor(255.f * (1.f - (Device.fTimeGlobal - UIStaticDiskIO_start_time) / 1.f)), 0, 255);
