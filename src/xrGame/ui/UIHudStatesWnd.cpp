...
void CUIHudStatesWnd::InitFromXml(CUIXml& xml, LPCSTR path)
{
    ...
    if (path)
    {
        ...
        xml.SetLocalRoot(xml.NavigateToNode(path, 0));
    }
    else
    {
        // Retail Shadow of Chernobyl keeps the actual HUD controls directly
        // below the document <window>, without the later-game hud_states
        // wrapper.  Use those real controls in their original coordinates.
        SetWndPos(Fvector2().set(0.0f, 0.0f));
        SetWndSize(Fvector2().set(UI_BASE_WIDTH, UI_BASE_HEIGHT));
        xml.SetLocalRoot(xml.GetRoot());
        Msg("~ ShoC HUD states: using retail root controls from %s", xml.m_xml_file_name);
    }
...
CUIStatic* CUIHudStatesWnd::GetIndicator(ALife::EInfluenceType type) const
{
    const auto it = m_indik.find(type);
    return it != m_indik.end() ? it->second : nullptr;
}
...
void CUIHudStatesWnd::UpdateIndicatorType(CActor* actor, ALife::EInfluenceType type)
{
    ...
    CUIStatic* indicator = GetIndicator(type);
    if (!indicator)
        return;
    ...
    if (hit_power < EPS)
    {
        ...
        if (CUITextureMaster::ItemExist(texture))
            indicator->InitTexture(texture);
        else if (CUITextureMaster::ItemExist(greenTexture))
            indicator->Show(false); // Use standard CoP scheme
        else
            indicator->SetTextureColor(c_white);
    ...
    if (hit_power <= protect)
    {
        ...
        if (CUITextureMaster::ItemExist(texture))
            ...
            indicator->SetTextureColor(c_green);
    ...
    if (hit_power - protect < m_zone_threshold[type])
    {
        ...
        if (CUITextureMaster::ItemExist(texture))
            ...
            indicator->SetTextureColor(c_yellow);
    ...
    if (CUITextureMaster::ItemExist(texture))
        ...
        indicator->SetTextureColor(c_red);
...
void CUIHudStatesWnd::SwitchLA(bool state, ALife::EInfluenceType type)
{
    ...
    if (state)
    {
        indicator->SetColorAnimation(m_lanim_name.c_str(), LA_CYCLIC | LA_TEXTURECOLOR);
        m_cur_state_LA[type] = true;
    }
    else
    {
        indicator->SetColorAnimation(nullptr, 0); //off
...
void CUIHudStatesWnd::DrawZoneIndicators()
{
    ...
    for (const ALife::EInfluenceType type :
        { ALife::infl_rad, ALife::infl_fire, ALife::infl_acid, ALife::infl_psi })
    {
        CUIStatic* indicator = GetIndicator(type);
