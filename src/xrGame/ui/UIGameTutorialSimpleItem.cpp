...
void CUISequenceSimpleItem::OnMousePress(int btn)
{
    ...
    // The startup sequence mixes videos with timed static publisher/logo
    // cards.  SDL reports a screen tap as a mouse press, so make those
    // pre-level cards dismissible just like the skippable videos.
    if (!g_pGameLevel)
    {
        m_flags.set(etiCanBeStopped, TRUE);
        // Do not call Stop() directly: IsPlaying() is time-based and would
        // otherwise keep the detached card current until its original timer
        // expired.  Expiring it makes CUISequencer::OnFrame run the normal
        // Next()/Stop()/Start() transition on the very next frame.
        m_time_start = float(Device.dwTimeContinual) / 1000.0f - m_time_length - EPS;
        return;
    }
#endif
    OnKeyboardPress(btn);
