...
void CUISequenceVideoItem::OnMousePress(int /*btn*/)
{
#ifdef XR_PLATFORM_ANDROID
    // SDL exposes a screen tap as the primary mouse button.  Marking the
    // current item as finished lets CUISequencer advance it on the next frame
    // through the normal Stop()/Next() path, exactly like an allowed Escape.
