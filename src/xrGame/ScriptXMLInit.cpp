...
CUITabControl* CScriptXmlInit::InitTab(LPCSTR path, CUIWindow* parent)
{
    ...
    // Retail ShoC option dialogs predate explicit tab IDs.  Their scripts
    // address tabs by index, so use the established compatibility IDs only
    // for ShoC data instead of rejecting ui_mm_opt.xml.
    CUIXmlInit::InitTabControl(m_xml, path, 0, pWnd, true, ShadowOfChernobylMode);
    _attach_child(pWnd, parent);
    return pWnd;
}
