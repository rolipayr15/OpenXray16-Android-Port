...
void CAI_Dog::Load(LPCSTR section)
{
    ...
    // ShoC asset sets exist with both the original misspelled prefix and the
    // corrected one.  The old AddAnim2 path selected whichever family was
    // actually present in the loaded OMF; keep that data-driven behaviour.
    if (!anim().AddAnim(eAnimDragCorpse, "stand_drage_", -1, &velocity_drag, PS_STAND, false))
    ...
    // These home-state families are not present in every ShoC dog OMF.  They
    // were optional before the 2025 animation-system refactor, which removed
    // the old target_may_not_exist flag.  Probe the real loaded motions and
    // only expose actions backed by an actual cycle.
    const bool hasHomeWalkSmelling =
        anim().AddAnim(eAnimHomeWalkSmelling, "stand_walk_smelling_", -1, &velocity_walk_smell, PS_STAND, false);
    const bool hasHomeWalkGrowl =
        anim().AddAnim(eAnimHomeWalkGrowl, "stand_growl_walk_", -1, &velocity_walk_growl, PS_STAND, false);
