...
void CSkeletonX::_Load(const char* N, IReader* data, u32& dwVertCount)
{
...
#ifdef __ANDROID__
    // Android Vulkan currently skins from the canonical OGF CPU vertices.
    // Keeping those vertices is also required by bone picking and wallmarks.
    constexpr bool forceSoftwareSkinning = true;
    u16 hw_bones_cnt = 0;
#else
    constexpr bool forceSoftwareSkinning = false;
    ...
    case 1:
    {
        ...
        if (!forceSoftwareSkinning && !RImplementation.o.ffp)
        {
            if (1 == bids.size())
            {
                // HW- single bone
    ...
    case 2:
    {
        ...
        if (!forceSoftwareSkinning && sw_bones_cnt <= hw_bones_cnt)
        {
            // HW- two weights
    ...
    const size_t count = forceSoftwareSkinning ? 0 : (RImplementation.o.ffp ? 0 : 1);
...
#ifdef __ANDROID__
bool CSkeletonX::AndroidSkinVertices(vertRender* destination, u32 vertexCount)
{
    if (!destination || !Parent || vertexCount == 0)
        return false;
    if (*Vertices1W)
        Skin1W(destination, *Vertices1W, vertexCount, Parent->bone_instances);
    else if (*Vertices2W)
        Skin2W(destination, *Vertices2W, vertexCount, Parent->bone_instances);
    else if (*Vertices3W)
        Skin3W(destination, *Vertices3W, vertexCount, Parent->bone_instances);
    else if (*Vertices4W)
        Skin4W(destination, *Vertices4W, vertexCount, Parent->bone_instances);
    else
        return false;
    return true;
}
