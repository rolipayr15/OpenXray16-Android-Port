...
#ifdef __ANDROID__
bool CSkeletonX_ext::_AndroidBuildGeometry(Fvisual& visual, u32 firstIndex, u32 indexCount,
    xr_vector<vertRender>& vertices, xr_vector<u16>& indices)
{
    if (!Parent || visual.vCount == 0 || indexCount == 0 || !visual.p_rm_Indices)
        return false;

    vertices.resize(visual.vCount);
    if (!AndroidSkinVertices(vertices.data(), visual.vCount))
        return false;

    auto* sourceIndices = static_cast<u16*>(visual.p_rm_Indices->Map(
        static_cast<size_t>(firstIndex) * sizeof(u16), static_cast<size_t>(indexCount) * sizeof(u16), true));
    if (!sourceIndices)
        return false;
...
bool CSkeletonX_ST::AndroidBuildGeometry(xr_vector<vertRender>& vertices, xr_vector<u16>& indices)
{
    return _AndroidBuildGeometry(*this, iBase, iCount, vertices, indices);
}

bool CSkeletonX_PM::AndroidBuildGeometry(xr_vector<vertRender>& vertices, xr_vector<u16>& indices)
{
    if (!nSWI.sw || nSWI.count == 0)
        return false;
    const FSlideWindow& window = nSWI.sw[0];
    return _AndroidBuildGeometry(*this, iBase + window.offset,
        static_cast<u32>(window.num_tris) * 3, vertices, indices);
}
#endif
