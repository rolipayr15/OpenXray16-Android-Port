...
bool CUIOptionsItem::IsOptCommandAvailable() const
{
    const bool available = Console && m_entry.size() && Console->GetCommand(m_entry.c_str());
    if (!available)
        Msg("~ UI option is unavailable for the active renderer: %s", m_entry.c_str());
    return available;
}
